TODO: names of team members
Jiaqi Yu
Lily Ru

TODO: contributions of each team member
1. MS1: 
Lily Ru:
- completed the complement, transpose, and ellipse functions (including associated helper functions)
- Write and test unit tests for is_in_ellipse

Jiaqi Yu:
- completed the emboss function (including associated helper functions)
- Wrote and test unit tests for clamp_gray and emboss_diff

Debug together

2. MS2: 
Lily Ru: Implemented assembly version of complement function
Jiaqi Yu: Implemented assembly version of transpose function

3. MS3:
Lily Ru: Implemented assembly version of ellipse function, is_in_ellipse, and clamp_gray helper functions
Jiaqi Yu: Implemented assembly version of emboss function and emboss diff helper function
